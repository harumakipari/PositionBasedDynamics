...
PrimitiveRenderer::PrimitiveRenderer(ID3D11Device* device)
{
    ...
    // 定数バッファ
    constantBuffer = std::make_unique<ConstantBuffer<CbScene>>(device);
    ...
    hr = device->CreateBuffer(&desc, nullptr, vertexBuffer.GetAddressOf());
    _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
}
    ...
    D3D11_PRIMITIVE_TOPOLOGY primitiveTopology)
{
    ...
    constantBuffer->data = cbScene;
    constantBuffer->Activate(dc, 0);
