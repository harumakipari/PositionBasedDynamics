...
namespace PBD
{
    ...
    void Solver::step(PBDWorld& world, float dt)
    {
        ...
        // 衝撃を受ける可能性のある形状をフレーム開始時に更新する
        for (auto& shape : world.collision_shapes)
        {
            shape->BeginFrame();
        }
    ...
    void Solver::project_contact_constraint(PBDWorld& world, const ContactConstraint& cc)
    {
        ...
        float correction = -cc.phi * (cc.stiffness / solver_iterations);

#if 0
        if (cc.b.type == body_type::static_shape)
        {
            world.collision_shapes[cc.b.index]->AddImpulse(
                cc.normal,
                correction,
                cc.contactPoint);
        }
