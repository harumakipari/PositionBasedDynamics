...
namespace PBD
{
        ...
        int voxel_resolution)
    {
        ...
        auto create_particle = [&](const XMFLOAT3& position, float inv_mass, float radius, int phase, voxel_state voxelState)
            {
                PBDParticle p{};
        ...
        for (auto position : transformed_positions)
        {
            create_particle(position, inverse_mass, particle_radius, 1, voxel_state::outside);
        }
                ...
                for (int y = 0; y < voxels.ny; y++)
                    for (int z = 0; z < voxels.nz; z++)
                    {
                        ...
                        if (voxel.state == voxel_state::inside)
                        {
                            ...
                            create_particle(position, inverse_mass, particle_radius, 1, voxel_state::inside);
                        }
                    }
        }
